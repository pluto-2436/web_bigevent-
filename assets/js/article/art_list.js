$(function () {
  ...
  function rederPage(total) {
    ...
    laypage.render({
      ...
      // 2.只要调用了laypage.render() 方法，就会触发 jump 回调 (导致死循环的原因)
      jump: function (obj, first) {
        //可以通过first 的值，来判断是通过哪种方式，触发的 jump 回调
        // 如果first 的值为：ture 那证明是方式2触发的 否则是方式 1 触发的
    ...
    layer.confirm('确认删除?', {icon: 3, title:'提示'}, function(index){
      $.ajax({
        ...
        success: function (res) {
          ...
          layer.msg('删除文章成功！')
          // 当数据删除完成后，需要判断这一页中，是否还有剩余的数据，如果没有剩余的数据了，则让页码值 -1 之后 ，再重新调用initTable 方法
          if (len === 1) {
            // 数值等于1，证明删除之后页面就没有数据了
            // 页码值最小必须是1
